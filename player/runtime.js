...
const SLOT_POLL_MS = 30_000; // per-binding-slot poll cadence
const RELOAD_MS = 6 * 60 * 60 * 1000; // 6h hard reload
...
async function fetchPlaylist() {
  try {
    ...
    if (cached && !state.playlist) applyPlaylist(cached, /* fromCache */ true);
  }
  setTimeout(fetchPlaylist, POLL_MS);
}
...
function applyPlaylist(data, fromCache = false) {
  ...
  if (fromCache) showBanner('Offline — showing cached playlist');
...
function rebuildSlotPolls() {
  ...
  for (const slug of Object.keys(state.slotTimers)) {
    if (!want.has(slug)) {
      clearInterval(state.slotTimers[slug]);
      delete state.slotTimers[slug];
      delete state.slotData[slug];
    }
  }
  // Start polls for new slots; do an immediate fetch so the first render has data
  for (const slug of want) {
    if (state.slotTimers[slug]) continue;
    fetchSlotData(slug);
    state.slotTimers[slug] = setInterval(() => fetchSlotData(slug), SLOT_POLL_MS);
  }
}
...
async function fetchSlotData(slug) {
  try {
    ...
    // Network blip — leave previous value intact.
  }
}
...
async function showNext() {
  ...
  if (state.visibleIdxList.length === 0) {
    ...
    showBanner('No visible slides right now.');
  ...
  const synced = computeSyncedIndex(state.playlist?.syncAnchor, state.slides.length);
  let nextIdx;
  if (synced != null && state.visibleIdxList.includes(synced)) {
  ...
  if (state.visibleIdxList.length === 1 && state.currentIdx === 0 && currentSlideEl && !state.forceRender) {
    const slide = state.slides[state.visibleIdxList[0]];
    const sec = Math.max(2, slide.duration ?? state.defaults.duration ?? 10);
    currentSlideStartedAt = performance.now();
    currentSlideDurationMs = sec * 1000;
    advanceTimer = setTimeout(showNext, currentSlideDurationMs);
    setTimeout(rebuildVisible, 60_000);
    return;
  }
...
async function renderSlide(slide) {
  ...
  disposeCurrent = () => { for (const d of disposers) { try { d(); } catch {} } };
...
function showBanner(text) {
  ...
  if (!b) {
    ...
    b.id = 'bb-banner';
    b.style.cssText = 'position:fixed;bottom:10px;right:10px;background:rgba(0,0,0,.55);color:#aaa;padding:6px 10px;font:12px monospace;border-radius:4px;z-index:9998;';
